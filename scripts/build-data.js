...
const csvFilename = 'pension-credits.csv';
...
function interrogateSPARQL(zones) {
    ...
    _.forEach(_.chunk(zones, chunk_size), (zone_chunk) => {
        queries.push(`
            ...
            SELECT ?period ?zone ?age ?gender ?element ?measure ?value
            WHERE {
                ?s qb:dataSet data:pension-credits ;
                   qb:measureType ?m ;
                   ?m ?value ;
                   dim:gender ?g ;
                   dim:pensionCreditElement ?e ;
                   dim:age ?a ;
                   ldim:refArea ?z ;
                   ldim:refPeriod ?p .
                ?g rdfs:label ?gender .
                ?z rdfs:label ?zone .
                ?e rdfs:label ?element .
                ?a rdfs:label ?age .
                ?p rdfs:label ?period .
